...
func TestNew(t *testing.T) {
	...
	if !reflect.DeepEqual(rb.buffer, make([]any, 32, 32)) {
...
func TestNewWithCapacity(t *testing.T) {
	...
	if !reflect.DeepEqual(rb.buffer, make([]any, 128, 128)) {
...
func TestPushBack(t *testing.T) {
	// given
	rb := NewWithCapacity(3)
	expectedBuffer := []any{8008135, nil, nil}
...
func TestPushBackExtendingCapacity(t *testing.T) {
	// given
	rb := NewWithCapacity(3)
	expectedBuffer := []any{1, 2, 3, 4, nil, nil}
	...
	// then
	if rb.size != 4 {
		t.Fatalf(`When PushBack() called then should have size: 4, has: %d`, rb.size)
	}
	if rb.capacity != 6 {
		t.Fatalf(`When PushBack() called then should have extended capacity: 6, has: %d`, rb.capacity)
	}
	if rb.front != 0 {
		t.Fatalf(`When PushBack() called then should have front: 0, has: %d`, rb.front)
	}
	if rb.tail != 3 {
		t.Fatalf(`When PushBack() called then should have tail: 3, has: %d`, rb.tail)
	}
	if !reflect.DeepEqual(rb.buffer, expectedBuffer) {
		t.Fatalf(`When PushBack() called then should have buffer: %v, has: %v`, expectedBuffer, rb.buffer)
	}
...
func TestPopFront(t *testing.T) {
	...
	if pop5 != nil {
		t.Fatalf(`When (5) PopFront() called then should return nil with error, has: %d`, pop2)
	}
	if err5 == nil {
	...
	if rb.size != 0 {
		t.Fatalf(`When PopFront() called then should have size: 0, has: %d`, rb.size)
	}
	if rb.capacity != 6 {
		t.Fatalf(`When PopFront() called then should have extended capacity: 6, has: %d`, rb.capacity)
	}
	...
	if rb.front != 4 {
		t.Fatalf(`When PopFront() called then should have front: 0, has: %d`, rb.front)
	}
	if rb.tail != 3 {
		t.Fatalf(`When PopFront() called then should have tail: 3, has: %d`, rb.tail)
	}
	if !reflect.DeepEqual(rb.buffer, expectedBuffer) {
		t.Fatalf(`When PopFront() called then should have buffer: %v, has: %v`, expectedBuffer, rb.buffer)
	}
}
...
func TestRingBehaviour(t *testing.T) {
	// given
	rb := NewWithCapacity(6)
	expectedBuffer := []any{3, 4, 5, 6, 1, 2}
...
func TestPeekFront(t *testing.T) {
	...
	// then
	if pf1 != nil {
		t.Fatalf(`PeekFront() should return nil when empty RingBuffer, has: %d`, pf1)
	}
	if err1 == nil {
		t.Fatalf(`PeekFront() should return error when empty RingBuffer, but it didn't`)
	}
	if pf2 != 12 {
		t.Fatalf(`PeekFront() should return 12 but returned: %d`, pf2)
	}
	if err2 != nil {
		t.Fatalf(`PeekFront() should not return error when non empty RingBuffer`)
	}
	if pf3 != 12 {
		t.Fatalf(`PeekFront() should return 12 but returned: %d`, pf3)
	}
	if err3 != nil {
		t.Fatalf(`PeekFront() should not return error when non empty RingBuffer`)
	}
	if pf4 != 21 {
		t.Fatalf(`PeekFront() should return 21 but returned: %d`, pf4)
	}
	if err4 != nil {
		t.Fatalf(`PeekFront() should not return error when non empty RingBuffer`)
	}
	if pf5 != nil {
		t.Fatalf(`PeekFront() should return nil when empty RingBuffer, has: %d`, pf5)
	}
	if err5 == nil {
		t.Fatalf(`PeekFront() should return error when empty RingBuffer, but it didn't`)
	}
...
func TestPeekTail(t *testing.T) {
	...
	// then
	if pf1 != nil {
		t.Fatalf(`PeekTail() should return nil when empty RingBuffer, has: %d`, pf1)
	}
	if err1 == nil {
		t.Fatalf(`PeekTail() should return error when empty RingBuffer, but it didn't`)
	}
	if pf2 != 11 {
		t.Fatalf(`PeekTail() should return 11 but returned: %d`, pf2)
	}
	if err2 != nil {
		t.Fatalf(`PeekTail() should not return error when non empty RingBuffer`)
	}
	if pf3 != 22 {
		t.Fatalf(`PeekTail() should return 22 but returned: %d`, pf3)
	}
	if err3 != nil {
		t.Fatalf(`PeekTail() should not return error when non empty RingBuffer`)
	}
	if pf4 != 33 {
		t.Fatalf(`PeekTail() should return 33 but returned: %d`, pf4)
	}
	if err4 != nil {
		t.Fatalf(`PeekTaiil() should not return error when non empty RingBuffer`)
	}
	if pf5 != 44 {
		t.Fatalf(`PeekTail() should return 44 when empty RingBuffer, has: %d`, pf5)
	}
	if err5 != nil {
		t.Fatalf(`PeekTaiil() should not return error when non empty RingBuffer`)
	}
